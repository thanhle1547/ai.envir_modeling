 ...
 * @see CCLine
 */
public class CCExtentSystem extends JPanel {
    /*
     * In this class there are two coordinate systems:
     * 
     * 1. A two-dimensional coordinate system for Java2D where x lies in the
     * interval [0, window width] and y lies in the interval [0, window height]
     * where the units of both x and y are pixels.
     * 
     * 2. An emulated two-dimensional coordinate system where x and y can lie in any
     * range definable by double precision numbers.
     * 
     * Throughout this class, Point is used to represent a point in system 1 while
     * Point2D is used to represent a point in system 2.
     * 
     * The translate.*(.)-methods are used to translate between the two systems.
     */
    private static final long serialVersionUID = 1L;
    
    private static CCExtentSystem instance;
    ...
    private CCRectangle workspaceBoundary;
    ...
    /*
     * Round this exact value to a value (of the same magnitude) that can be written
     * with very few decimals (or lots of trailing zeroes.)
     * 
     ...
     * <p>
     * xy-axes, units and a grid is drawn by default. This can be changed with the
     * following methods:
     * 
     * <pre>
     ...
     * respectively.
     */
    private CCExtentSystem(double minX, double minY, double maxX, double maxY) {
        this.minX = minX;
        this.minY = minY;
        this.maxX = maxX;
        this.maxY = maxY;
     ...
     * 
     * @param line a {@code CCLine} object.
     */
    ...
    public void add(CCExtentPolygon polygon) {
    ...
    /*
     * Draw the axes and unit lines in the best looking way possible for the given
     * x- and y-ranges.
     */
    ...
    private void drawLineSlope(Graphics2D g2d, CCLine line) {
        ...
        /*
         * Possible intercept-pairs: 1. minX and minY 2. minX and maxY 3. minX and maxX
    ...
    private void drawStartEndPoints(Graphics2D g2d) {
        for (CCExtentPoint point : this.sePoint) {
            Point p = translate(new Point2D.Double(point.x, point.y));
            double  inRadius = point.getStyle().getInnerRadius(),
                    inDiameter = inRadius * 2,
                    outRadius = point.getStyle().getOuterRadius(),
                    outDiameter = outRadius * 2;

            g2d.setPaint(point.getStyle().getInnerPaint());
            g2d.setStroke(point.getStyle().getInnerStroke());
            Ellipse2D r2d = new Ellipse2D.Double(p.x - inRadius, p.y - inRadius, inDiameter, inDiameter);
            g2d.draw(r2d);
            if (point.getStyle().getInnerPaint() != null)
                g2d.fill(r2d);
            
            g2d.setPaint(point.getStyle().getOuterPaint());
            g2d.setStroke(point.getStyle().getOuterStroke());
            r2d = new Ellipse2D.Double(p.x - outRadius, p.y - outRadius, outDiameter, outDiameter);
            g2d.draw(r2d);
            if (point.getStyle().getOuterPaint() != null)
                g2d.fill(r2d);
        }
    }

    private void drawWorkspaceBoundary(Graphics2D g2d) {
        if (this.workspaceBoundary.getWidth() <= 0 || this.workspaceBoundary.getHeight() <= 0)
            return;

        double[] dxpoints = this.workspaceBoundary.getPolygonXPoints(),
                 dypoints = this.workspaceBoundary.getPolygonYPoints();
        int num = dxpoints.length;
        int[] ixpoints = new int[num];
        int[] iypoints = new int[num];

        for (int i = 0; i < num; i++) {
            ixpoints[i] = translateX(dxpoints[i]);
            iypoints[i] = translateY(dypoints[i]);
        }
        Polygon p = new Polygon(ixpoints, iypoints, num);

        g2d.setStroke(this.workspaceBoundary.getStyles().getStroke());
        g2d.setPaint(this.workspaceBoundary.getStyles().getPaint());
        if (this.workspaceBoundary.getStyles().getPaint() != null)
            g2d.fill(p);

        g2d.setColor(this.workspaceBoundary.getStyles().getBorderColor());
        g2d.draw(p);
    }

    /* Draw a polygon */
    private void drawPolygon(Graphics2D g2d, CCExtentPolygon poly) {
    ...
    private void drawXUnitLine(Graphics2D g2d, BigDecimal val) {
        ...
        boolean small = val.compareTo(big) < 0 && val.compareTo(big.negate()) > 0;

        /*
         * When val is not "small", BigDecimal's toString does not use scientific
         * notation, so Double's toString is used instead.
         */
        String strval;
        if (small)
            strval = val.toString();
        else
            strval = Double.toString(val.doubleValue());
    ...
    private void drawXUnitLines(Graphics2D g2d) {
        /*
         * The value at each unit line will now be defined as i * vbuX. We need to find
         * the value of i such that i * vbuX is the value at the first visible unit
         * line.
         */
        ...
        for (int i = idx; i <= end; i++)
            drawXUnitLine(g2d, BigDecimal.valueOf(i).multiply(vbuX, prec));
    }
    ...
    private void drawYUnitLines(Graphics2D g2d) {
        ...
        for (int i = idx; i <= end; i++)
            drawYUnitLine(g2d, BigDecimal.valueOf(i).multiply(vbuY, prec));
    }
    ...
    private BigDecimal findScale(double num) {
        ...
        if (quot > 5.0)
            return scale.multiply(BigDecimal.TEN, prec);
        if (quot > 2.0)
            return scale.multiply(BigDecimal.valueOf(5), prec);
        if (quot > 1.0)
            return scale.multiply(BigDecimal.valueOf(2), prec);
        else
            return scale;
    }
    ...
    public CCExtentPoint getEndPoint() {
        return sePoint.get(1);
    }

    public CCRectangle getWorkspaceBoundary() {
        return workspaceBoundary;
    }

    /**
     * @see <a href="https://viblo.asia/p/singleton-pattern-trong-java-aWj53838K6m">
     *          Singleton Pattern trong java
     *      </a>
     */
    public static CCExtentSystem getInstance() {
        if (instance == null)
            synchronized(CCExtentSystem.class) {
                if (instance == null)
                    instance = new CCExtentSystem();
            }
        return instance;
    }

    /**
     * Moves the entire visible area of the current system. The visible area will be
     * x and y in [loX, hiX] and [loY, hiY], respectively.
     * 
     * @param loX Lowest visible value of x.
     * @param hiX Highest visible value of x.
     * @param loY Lowest visible value of y.
     * @param hiY Highest visible value of y.
     */
     ...
     * 
     * @param paint {@code Paint} to paint the axes with.
     */
     ...
     * 
     * @param stroke {@Code Stroke} to paint the axes with.
     */
     ...
     * 
     * @param visible If true, the axes are drawn. Otherwise they are hidden.
     */
     ...
     * 
     * @param paint {@code Paint} to paint the x-axis with.
     */
     ...
     * 
     * @param stroke {@Code Stroke} to paint the x-axis with.
     */
     ...
     * 
     * @param visible If true, the x-axis is drawn. Otherwise it is hidden.
     */
     ...
     * 
     * @param paint {@code Paint} to paint the y-axis with.
     */
     ...
     * 
     * @param stroke {@Code Stroke} to paint the y-axis with.
     */
     ...
     * 
     * @param visible If true, the y-axis is drawn. Otherwise it is hidden.
     */
     ...
     * 
     * @param paint {@code Paint} to paint the grid with.
     */
     ...
     * 
     * @param ratio The number of grid lines between each unit line.
     */
     ...
     * 
     * @param stroke {@Code Stroke} to paint the grid with.
     */
     ...
     * 
     * @param visible If true, the grid is drawn. Otherwise it is hidden.
     */
     ...
     * 
     * @param paint {@code Paint} to paint the horizontal grid-lines with.
     */
     ...
     * 
     * @param paint {@code Stroke} to paint the horizontal grid-lines with.
     */
     ...
     * 
     * @param visible If true, the horizontal grid lines are drawn. Otherwise they
     *                are hidden.
     */
     ...
     * 
     * @param paint {@code Paint} to paint the vertical grid-lines with.
     */
     ...
     * 
     * @param stroke {@Code Stroke} to paint the vertical grid-lines with.
     */
     ...
     * 
     * @param visible If true, the vertical grid-lines are drawn. Otherwise they are
     *                hidden.
     */
     ...
     * 
     * @param paint {@code Paint} to paint the unit lines with.
     */
     ...
     * 
     * @param stroke {@code Stroke} to paint the unit lines lines with.
     */
     ...
     * 
     * @param visible If true, unit lines are drawn on the axes. Otherwise they are
     *                hidden.
     */
     ...
     * 
     * @param paint {@code Paint} to paint the unit lines on the x-axis with.
     */
     ...
     * 
     * @param stroke {@code Stroke} to paint the unit lines on the x-axis lines
     *               with.
     */
     ...
     * 
     * @param visible If true, unit lines are drawn on the x-axis. Otherwise they
     *                are hidden.
     */
     ...
     * 
     * @param paint {@code Paint} to paint the unit lines on the y-axis with.
     */
     ...
     * 
     * @param stroke {@code Stroke} to paint the unit lines on the y-axis lines
     *               with.
     */
     ...
     * 
     * @param visible If true, unit lines are drawn on the y-axis. Otherwise they
     *                are hidden.
     */
    ...
    /**
     * Set whether the coordinate system is movable with the mouse, i.e. the scope
     * of the system changes as the the mouse is clicked, held and dragged over the
     * system.
     * 
     * @param movable If true, move is possible.
     */
     ...
     * </pre>
     * 
     * @param niceGraphics If true, use nice graphics.
     */
    ...
    /**
     * Set whether it is possible to zoom in/out in the coordinate system by
     * scrolling the mouse wheel.
     * 
     * @param zoomable If true, zoom is possible.
     */
    ...
    /**
     * Call redraw() after to see the changed
     */
    public void setPolygon(int index, CCExtentPolygon polygon) {
        this.polygons.set(index, polygon);
    }

    public void setStartPoint(CCExtentPoint startPoint) {
        try {
            this.sePoint.set(0, startPoint);
        } catch (IndexOutOfBoundsException ex) {
            this.sePoint.add(startPoint);
        }
    }

    public void setEndPoint(CCExtentPoint endPoint) {
        try {
            this.sePoint.set(1, endPoint);
        } catch (IndexOutOfBoundsException ex) {
            this.sePoint.add(endPoint);
        }
    }

    public void setWorkspaceBoundary(CCRectangle workspaceBoundary) {
        this.workspaceBoundary = workspaceBoundary;
        repaint();
    }

    public void setPolygonHighlightState(Integer id, boolean status) {
        polygons.get(id).setFill(status ? Color.BLUE : Color.PINK);
    }

    public CCExtentSystem setMinMax(double minX, double minY, double maxX, double maxY) {
        this.minX = minX;
        this.minY = minY;
        this.maxX = maxX;
        this.maxY = maxY;
     ...
     * 
     * Subtract from getHeight() since increasing y goes south in System 1 but north
     * in System 2.
     */
    ...
    private void updatePosition() {
        ...
        /*
         * Place origin along the edges of the screen if (0, 0) is not in the visible
         * area.
         */
        if (minX >= 0)
            ox = minX;
    ...
    /*
     * Zoom into the visible area relevant to the current position by keeping the
     * center the same.
     */
    ...
    /**
     * A {@code MouseWheelListener} making it possible to zoom in and out of the
     * coordinate system using the mouse wheel.
     */
